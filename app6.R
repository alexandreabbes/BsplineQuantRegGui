...
ui <- fluidPage(
  ...
  titlePanel(
    h1("BsplineQuantReg - Quantile Regression with BSplines under Shape Constraints",
    ...
    sidebarPanel(
      ...
      conditionalPanel(
        ...
        conditionalPanel(
          ...
          radioButtons("der3", "3rd Dérivative:",
                       choices = c("✗" = "0", "+" = "1", "-" = "-1"),
                       selected = "0", inline = TRUE)
        )
      ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$test_data, {
    withProgress(message = "Generating...", {
      ...
      values$regions <- list()
      showNotification("Testing data générated", type = "message")
    })
  })
  ...
  observeEvent(input$temp_data, {
    withProgress(message = "Loading...", {
      ...
      values$xtab <- x
      values$ytab <- y
      values$data_name <- "Temperatures (1880-1992)"
      values$fit <- NULL
      values$curve_lines <- list()
      values$regions <- list()
      ...
      values$knots <- knots
      showNotification("Temperatures data loded", type = "message")
    })
  })
  ...
  observeEvent(input$generate_custom, {
    tryCatch({
      ...
      values$xtab <- x
      values$ytab <- y
      values$data_name <- "Custumised Function"
      values$fit <- NULL
      values$curve_lines <- list()
      values$regions <- list()
      showNotification("Generated Data", type = "message")
    }, error = function(e) {
  ...
  observeEvent(input$add_knot_mode, {
    ...
    if (values$adding_knot) {
      showNotification("Mode add knots: click on the graphic", type = "message")
      updateActionButton(session, "add_knot_mode", label = "TOP")
    } else {
      updateActionButton(session, "add_knot_mode", label = "Add knots")
    }
  })
