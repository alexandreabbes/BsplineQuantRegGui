  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      tabsetPanel(
        ...
        tabPanel("📈 Visualisation", value = "plot",
                 ...
                 fluidRow(
                   ...
                   column(2,
                          ...
                          #br()
                          #,
                          #downloadButton("download_png", "📊 Exporter PNG", class = "btn-sm btn-success", width = "100%")
                   #)
                 ),
                 br(),
                 fluidRow(
        ...
        #,
         #        br(),)
                 downloadButton("download_code", "💾 Exporter code R", class = "btn-sm btn-success")  ),
...
server <- function(input, output, session) {
  ...
  observeEvent(event_data("plotly_selected", source = "plot"), {
    if (input$constraint_mode == "region" && values$selecting_region) {
      ...
      if (!is.null(selected) && nrow(selected) > 0) {
        # Prendre les points sélectionnés
  ...
  observeEvent(input$run, {
    ...
    withProgress(message = "Régression en cours...", {
      ...
      # Pour degré 3, der3 est un vecteur de longueur kn
      # Pour degré 4, der3 est un vecteur de longueur kn+1
      # Pour degré 1 ou 2, der3 est ignoré
      if (degree < 3) der3 <- rep(0, length(values$knots) - 1)
  ...
  output$download_code <- downloadHandler(
    filename = function() {
      paste0("BsplineQuantReg_code_", format(Sys.time(), "%Y%m%d"), ".R")
    },
    ...
    #store the plot as it is created
